...
plt.xticks([1,2], ['Federal', 'Municipal'], fontsize=10)
...
def percent(df1, df2, alpha):
    ...
    for i in range(10000):
        ...
        values1[i] = sample1.mean()
        values2[i] = sample2.mean()
    mean_diff = values1 - values2
    p_value = np.mean(mean_diff <= 0)
...
df[df['TEAM'].isin(['Charlotte Hornets', 'Phoenix Suns'])].groupby('TEAM')['SALARY'].mean()
...
def t_obs(data):
    ...
    hornets_salaries = data[data['TEAM'] == 'Charlotte Hornets']['SALARY']
    suns_salaries = data[data['TEAM'] == 'Phoenix Suns']['SALARY']

    # Calculando as médias
    mean_hornets = hornets_salaries.mean()
    mean_suns = suns_salaries.mean()

    # Calculando as variâncias
    var_hornets = hornets_salaries.var()
    var_suns = suns_salaries.var()

    # Calculando os tamanhos das amostras
    n_hornets = len(hornets_salaries)
    n_suns = len(suns_salaries)

    # Calculando a estatística de teste corretamente
    pooled_var = ((n_hornets - 1) * var_hornets + (n_suns - 1) * var_suns) / (n_hornets + n_suns - 2)
    t = (mean_hornets - mean_suns) / np.sqrt(pooled_var * (1/n_hornets + 1/n_suns))
...
def resposta(data):
   # Separando as amostras
    ...
    t_statistic, p_value = ttest_ind(hornets_salaries, suns_salaries, equal_var=False)
    ...
    if p_value < alpha:
        return False  # Diferença significativa entre os salários
    else:
        return True  # A diferença pode ser explicada pelo acaso
...
def t_obs2(data):
    cavs_height = data[data['team_abbreviation'] == 'CLE']['player_height'].values
    warriors_height = data[data['team_abbreviation'] == 'GSW']['player_height'].values

    # Calculando a média das alturas
    mean_cavs_height = np.mean(cavs_height)
    mean_warriors_height = np.mean(warriors_height)

    # Calculando a diferença das médias
    diff_means = mean_cavs_height - mean_warriors_height
...
def critical_values(diffs):
  # Definindo o nível de significância
    ...
    # Calculando os percentis correspondentes ao nível de significância
    lower_percentile = (alpha / 2) * 100
    upper_percentile = (1 - alpha / 2) * 100

    # Calculando os valores críticos
    critical_lower = np.percentile(diffs, lower_percentile)
    critical_upper = np.percentile(diffs, upper_percentile)
...
def p_value(t_obs, diffs):
    """
    Calcula o p-valor, ou seja, a probabilidade de termos uma diferença maior que a estatística de teste.

    Parâmetros:
    t_obs (float): Estatística de teste observada.
    diffs (array): Array contendo as diferenças entre as médias das amostras permutadas.

    Retorna:
    float: O p-valor calculado.
    """
    # Conta quantas diferenças são maiores que a estatística de teste observada
    count = sum(diff > t_obs for diff in diffs)

    # Calcula a proporção de diferenças maiores que a estatística de teste observada
    p_val = count / len(diffs)

    return 1 - p_val

result = t_obs2(df)
foo = p_value(result, diffs)
assert_equal(round(foo, 4), 0.2578)
...
def resposta2(diffs, t_obs):
    ...
    p_val = p_value(t_obs, diffs)

    # Verificando se rejeita ou não a hipótese nula com base no p-valor
    if p_val < alpha:
        return True  # Rejeita a hipótese nula
    else:
        return False  # Não rejeita a hipótese nula
