...
Original file is located at
    https://colab.research.google.com/drive/1I53pUI7gOpL8g1B8ulib_NHcLPSgzhQj
...
def loss_um_ponto(x_i, y_i, alpha, beta):
    # Calcular a predição do modelo linear
    y_pred = alpha + beta * x_i

    # Calcular o erro (diferença entre o valor real e o valor previsto)
    erro = y_i - y_pred

    # Calcular o erro quadrático
    erro_quadratico = erro ** 2

    # Retornar o erro quadrático
...
def gradient(x_i, y_i, alpha, beta):
    ...
    # Calcular o gradiente com relação a α
    da = -2 * erro

    # Calcular o gradiente com relação a β
    db = -2 * erro * x_i

    # Retornar os gradientes em uma lista [da, db]
    return [da, db]
...
def descent(x, y, param, lambda_=0.0001, niter=1000):
    # Inicializar os parâmetros alpha e beta
    alpha, beta = param
    ...
    for _ in range(niter):
        ...
        for i in range(n):
            # Calcular os gradientes para o ponto (x[i], y[i])
            da, db = gradient(x[i], y[i], alpha, beta)
            # Acumular os gradientes
            total_da += da
            total_db += db

        # Atualizar os parâmetros usando os gradientes acumulados
        alpha -= (lambda_) * total_da
        beta -= (lambda_) * total_db
...
def predict(x, param):
  ...
  # YOUR CODE HERE
  # Extraindo os parâmetros alpha e beta da lista param
  alpha, beta = param

  # Calculando as predições para cada valor em x usando o modelo linear
  y_pred = alpha + beta * x
...
def sse(x, y, param):
  ...
  erros_quadraticos = (y - y_pred) ** 2

  # Calculando a soma dos erros quadráticos (SSE)
  soma_erros_quadraticos = erros_quadraticos.sum()

  # Retornando a soma dos erros quadráticos (SSE)
  return soma_erros_quadraticos
...
def idade_universo(sd_x, sd_y):
    ...
    # Calcular os desvios padrão não normalizados de x e y
    std_x = np.std(x, ddof=1)
    std_y = np.std(y, ddof=1)

    # Calcular beta (coeficiente da regressão)
    beta = r * (std_y / std_x)

    # Calcular a idade do universo (em bilhões de anos)
    age_universe = 1 / beta
    return age_universe/10**3

print(idade_universo(x, y))
