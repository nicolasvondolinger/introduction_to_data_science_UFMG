...
def delay(df) -> tuple:
    return (df.dropna().DepDelay.mean(), df.dropna().ArrDelay.mean())
...
def high_delay(df):
    ...
    good['FlightTime'] = np.zeros(good.shape[0])
    idx_same_day = good.CRSArrTime >= good.CRSDepTime
    good.loc[idx_same_day, 'FlightTime'] = good.CRSArrTime - good.CRSDepTime
    good.loc[~idx_same_day, 'FlightTime'] = 2400 - good.CRSDepTime + good.CRSArrTime

    route_with_longest_flight = good.groupby(['OriginAirportName', 'DestAirportName'])['FlightTime'].mean().idxmax()

    return route_with_longest_flight
...
df_filtered = df[df['Cancelled'] == 0]
plt.scatter(df_filtered.DepDelay, df_filtered.ArrDelay, edgecolor='k', alpha=0.75)