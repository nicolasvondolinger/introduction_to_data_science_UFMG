...
Original file is located at
    https://colab.research.google.com/github/icd-ufmg/icd-ufmg.github.io/blob/master/listas/l8/sol.ipynb
...
def sum_of_squared_errors(alpha, beta, x, y):
    squared_errors = [(error(alpha, beta, x_i, y_i))**2 for x_i, y_i in zip(x, y)]
...
def linear_regression(x, y):
    ...
    # Calculate the numerator and denominator for beta
    numerator = sum((x_i - mean_x) * (y_i - mean_y) for x_i, y_i in zip(x, y))
    denominator = total_sum_of_squares(x)
...
def plot_residuals(x, y, alpha, beta):
    """
...
def error_verification(alpha, beta, x, y):
  ...
  # (maiores, menores)
  errors = [error(alpha, beta, x_i, y_i) for x_i, y_i in zip(x, y)]
  greater_than_zero = sum(1 for e in errors if e > 0)
  less_than_zero = sum(1 for e in errors if e < 0)
...
# seu codigo aqui
# gabarito a seguir
def plot_probability_plot(errors):
    """
    Plot a probability plot (Q-Q plot) for the errors.
    """
    ss.probplot(errors, dist="norm", plot=plt)
...
# Get errors
errors = [error(a1, b1, x_i, y_i) for x_i, y_i in zip(y, x)]

# Plot probability plot
plot_probability_plot(errors)
...
# Get errors
errors = [error(a2, b2, x_i, y_i) for x_i, y_i in zip(y, x)]
