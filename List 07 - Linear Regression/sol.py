...
def error(alpha, beta, x, y):
    predicted_value = predict(alpha, beta, x)
    # Calcula o erro
    error_value = y - predicted_value
    # Retorna o erro
    return error_value
...
x = copy['midparentHeight'].copy()
y = copy['childHeight'].copy()
alpha = 1
beta = 0.5

# definir alpha, beta, x e y fora da funcao
def mean_error(alpha, beta, x, y):
    # Inicializa a soma dos erros
    total_error = 0
    # Calcula o erro para cada par (x, y)
    for x_i, y_i in zip(x, y):
        total_error += error(alpha, beta, x_i, y_i)
    # Calcula o erro médio
    mean_error_value = total_error / len(x)
    # Retorna o erro médio
...
# Gera os valores de x para a reta
x_range = np.linspace(min(x), max(x), 100)

# Calcula os valores de y para a reta
y_predicted = alpha + beta * x_range

# Plota a reta
plt.plot(x_range, y_predicted, color='magenta')
...
# prompt: A seguir, mostre o gráfico de dispersão dos dados e a reta gerada pelo modelo. (SEM CORREÇÃO AUTOMÁTICA)
...
def sum_of_squared_errors(alpha, beta, x, y):
    # Inicializa a soma dos erros quadrados
    total_squared_error = 0
    # Calcula o erro quadrático para cada par (x, y)
    for x_i, y_i in zip(x, y):
        total_squared_error += error(alpha, beta, x_i, y_i) ** 2
...
def least_squares_fit(x, y):
   # Calcula a média de x e y
    ...
    # Calcula o valor ótimo de beta
    beta = covariance_xy / variance_x

    # Calcula o valor ótimo de alpha
...
# Gerando os valores preditos para a reta do modelo
y_pred = alpha + beta * x

# Plotando o gráfico de dispersão e a reta do modelo
plt.figure(figsize=(10, 6))
plt.scatter(x, y, color='blue', label='Dados reais')
plt.plot(x, y_pred, color='red', label='Reta de regressão')
plt.xlabel('Altura média dos pais')
plt.ylabel('Altura da criança')
...
def r_squared(alpha, beta, x, y):
  # Calcula os valores preditos
    y_pred = predict(alpha, beta, x)

    # Calcula SS_res (soma dos quadrados dos resíduos)
    ss_res = np.sum((y - y_pred) ** 2)

    # Calcula SS_tot (soma total dos quadrados)
    mean_y = np.mean(y)
