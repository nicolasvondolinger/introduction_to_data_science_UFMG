...
def CI(data):
     # Filtrando as escolas com DEPENDENCIA_ADMINISTRATIVA igual a 'Estadual'
    escolas_estaduais = data[data['DEPENDENCIA_ADMINISTRATIVA'] == 'Estadual']

    # Calculando a média e o desvio padrão da variável 'NOTA_MEDIA_ESCOLA' para as escolas estaduais
    media = escolas_estaduais['NOTA_MEDIA_ESCOLA'].mean()
    desvio_padrao = escolas_estaduais['NOTA_MEDIA_ESCOLA'].std()

    # Calculando o tamanho da amostra
    n = len(escolas_estaduais)

    # Calculando o intervalo de confiança
    t_score = t.ppf(0.995, df=n-1) # 99% de confiança, dois lados
    margin_of_error = t_score * (desvio_padrao / np.sqrt(n))
    lower_bound = media - margin_of_error
    upper_bound = media + margin_of_error
...
ecdf = ECDF(data['TAXA_DE_APROVACAO'].tolist())

# Valores para os quais deseja calcular a CDF
x_values = np.linspace(min(data['TAXA_DE_APROVACAO']), max(data['TAXA_DE_APROVACAO']), 1000)

# Calculando a CDF para os valores x
cdf_values = ecdf(x_values)

# Plotando a CDF
plt.plot(x_values, cdf_values, label='ECDF')
plt.xlabel('Taxa de Aprovação')
plt.ylabel('Probabilidade acumulada')
plt.title('CDF da Taxa de Aprovação Geral')
plt.legend()
plt.grid(True)
plt.show()
...
def bootstrap_mean(n_sub, alpha):
    ...
    for i in range(n_sub):
        ...
        mean = sample.query('DEPENDENCIA_ADMINISTRATIVA == "Federal"')['NOTA_MEDIA_ESCOLA'].mean()
        means.append(mean)

    # Calculando os percentis para o intervalo de confiança
    alpha_percentile = (1 - alpha) / 2
    lower_percentile = alpha_percentile * 100
    upper_percentile = (1 - alpha_percentile) * 100
